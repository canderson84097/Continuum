...
class PostController {
    ...
    func fetchComments(for post: Post, completion: @escaping ([Comment]?) -> Void) {
        ...
        let predicate = NSPredicate(format: "%K == %@", CommentConstants.postReferenceKey, postRef)
        ...
        CKContainer.default().publicCloudDatabase.perform(query, inZoneWith: nil) { (records, error) in
            if let error = error {
                print("Error fetching comments from iCloud \(#function) \(error.localizedDescription) \(error)")
                return completion(nil)
            }
            guard let records = records else { completion(nil); return }
    ...
    func incrementCommentCount(for post: Post, completion: ((Bool) -> Void)?) {
        ...
        modifyOperation.modifyRecordsCompletionBlock = { (records, _, error) in
            if let error = error {
                print("\(error.localizedDescription) \(error) in \(#function)")
                completion?(false)
                return
            } else {
                completion?(true)
    ...
    func subscribeToNewPosts(completion: ((Bool, Error?) -> Void)?) {
        ...
        // subscriptionID: "AllPosts" breaks code
    ...
    func addSubscriptionTo(commentsForPost post: Post, completion: ((Bool, Error?) -> ())?) {
        ...
        let notificationInfo = CKSubscription.NotificationInfo()
        notificationInfo.alertBody = "A new comment was added on a post you follow."
        notificationInfo.shouldSendContentAvailable = true
    ...
    func removeSubscriptionTo(commentsForPost post: Post, completion: ((Bool) -> ())?) {
        ...
        CKContainer.default().publicCloudDatabase.delete(withSubscriptionID: subscriptionID) { (subscription, error) in
            if let error = error {
                print("There was an error in \(#function) : \(error) : \(error.localizedDescription)")
                completion?(false)
            } else {
        ...
        checkForSubscription(to: post) { (isSubscribed) in
            if isSubscribed {
                self.removeSubscriptionTo(commentsForPost: post, completion: { (success) in
                    if success {
                        print("Successfully removed subscription to post with caption: \(post.caption)")
                        completion?(true, nil)
                    } else {
                        print("Error removing the subscription to post with post caption: \(post.caption)")
                ...
                self.addSubscriptionTo(commentsForPost: post, completion: { (success, error) in
                    ...
                    if success {
                        print("Successfully add the subscription to the post with caption: \(post.caption)")
                        completion?(false, nil)
                    }
                })
            }
